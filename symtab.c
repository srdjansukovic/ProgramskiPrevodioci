...
int insert_symbol(char *name, unsigned kind, unsigned type, 
                  unsigned atr1, unsigned atr2, int iter){
  ...
  symbol_table[index].atr2[iter] = atr2;
  return index;
}
...
int insert_literal(char *str, unsigned type) {
  ...
  idx = insert_symbol(str, LIT, type, NO_ATR, NO_ATR, 0);
  return idx;
}
...
void set_atr2(int index, unsigned atr2, int iter) {
  if(index > -1 && index < SYMBOL_TABLE_LENGTH)
    symbol_table[index].atr2[iter] = atr2;
}

unsigned get_atr2(int index, int iter) {
  if(index > -1 && index < SYMBOL_TABLE_LENGTH)
    return symbol_table[index].atr2[iter];
  return NO_ATR;
}
...
void print_symtab(void) {
  ...
  for(i = 0; i < first_empty; i++) {
    printf("\n%2d %-19s %-4s %4d  %4d  (%d %d %d %d %d %d %d %d %d %d)", i, // prints first ten atributes, just for test
    symbol_table[i].name, 
...
void init_symtab(void) {
  ...
  for(i = 0; i < 14; i++) {
    sprintf(s, "%%%d", i);
    insert_symbol(strdup(s), REG, NO_TYPE, NO_ATR, NO_ATR, 0);
  }
}
